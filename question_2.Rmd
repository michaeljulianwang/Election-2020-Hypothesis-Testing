...
tableDat <- dat %>% mutate(diff = Biden_Meter - Kamala_Meter) %>% summarise(`Record Count` = n(),
                              `Average Kamala Harris Thermometer` = round(mean(Kamala_Meter), 2),
                              `Average Joe Biden Thermometer` = round(mean(Biden_Meter), 2),
                              `Average Difference` = round(mean(diff), 2))
kable(tableDat, caption = 'Data Distribution by Candidate')
```

From the table below, we see that, along with a small difference in mean, there isn't a strong visual difference between the distributions of thermometer by Joe Biden and Kamala Harris though Kamala Harris has a higher distribution near 50.

```{r graphic distribution, echo=FALSE, warning=TRUE, message=FALSE}
stackedDat <- melt(dat %>% select(Kamala_Meter, Biden_Meter)) %>%
  mutate(variable = case_when(variable == 'Kamala_Meter' ~ 'Kamala Harris',
                              TRUE ~ 'Joe Biden'))

ggplot(stackedDat, aes(variable, value)) +
  geom_violin() +
  labs(title="Violin of Thermometer",
       x="Candidate",
       y="Candidate Thermometer")
```

```{r graphic distribution type2, echo=FALSE, warning=TRUE, message=FALSE}
dat %<>% mutate(diff = Kamala_Meter - Biden_Meter)

ggplot(dat, aes(x=diff)) +
  geom_histogram(color="black", fill="white") +
  geom_vline(aes(xintercept=mean(diff)),
            color="blue", linetype="dashed", size=1) +
  labs(title="Difference Between Kamala Harris & Joe Biden",
...
* Metric scale - The thermometer is a metric scale?
...
wilcox.test(dat$Kamala_Meter, dat$Biden_Meter, paired = TRUE, alternative = "two.sided")
```
